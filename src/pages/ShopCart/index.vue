  ...
  <div class="cart">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart, index) in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="delteCartById(cart)">删除</a>
            <br />
            <a href="#none">移到收藏</a>
          </li>
        </ul>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllChecked && cartInfoList.length > 0" @change="updateAllItemChecked" />
        <span>全选</span>
      </div>
      <div class="option">
...
export default {
  ...
  methods: {
    ...
    // 修改产品购物车数量 type:三个按钮;disNum:变化的量;cart：点击的产品
    ...
    //修改全部产品选定状态
    async updateAllItemChecked(event) {
      try {
        let isChecked = event.target.checked ? '1' : '0'
        //派发action
        await this.$store.dispatch('updateAllItemChecked', isChecked)
        this.getData()
      } catch (error) {
        alert(error.message)
      }
