          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr, index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{ active: attr.isChecked == 1 }" v-for="(attr, index) in spuSaleAttr.spuSaleAttrValueList" :key="attr.id" @click="changeActive(attr, spuSaleAttr.spuSaleAttrValueList)">{{ attr.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
  ...
  methods: {
    ...
    async addShopCart() {
      ...
      try {
        ...
        // 进行路由跳转 带参 query的话路径会被转换字符串 可行。本次使用（localstorage-支持持久化5M存储；会话存储，并非持久，会话结束数据消失）
        // 采用会话存储，仅展示一次即可，无需本地存储。
        // 简单数据(skuNum)通过query传递给下一个路由组件，复杂数据放入会话存储
        // 本地 会话 一般存储字符串
