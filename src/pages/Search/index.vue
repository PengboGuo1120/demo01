            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="changeOrder('1')">
                  <a>综合 <i v-show="isOne" :class="{ 'iconfont icon-up': isAsc, 'iconfont icon-down': isDesc }"></i> </a>
                </li>
                <li :class="{ active: isTwo }" @click="changeOrder('2')">
                  <a>价格 <i v-show="isTwo" :class="{ 'iconfont icon-up': isAsc, 'iconfont icon-down': isDesc }"></i> </a>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  //在发请求之前传送给服务器的searchParams参数需要发生变化
  beforeMount() {
    //Object.assign方法去书写,用于合并对象
  ...
  computed: {
    // mapGetters 传递数组 getters内部不会划分模块
  ...
  watch: {
    $route(newValue, oldValue) {
      //再次发请求之前 需要重新整理发送请求的参数
      Object.assign(this.searchParams, this.$route.query, this.$route.params)
      //再次发送请求
      this.getData()
      //请求完毕后 清楚本次三级分类数据清空，用于接收下一次的数据
      ;(this.searchParams.category1Id = ''), (this.searchParams.category2Id = ''), (this.searchParams.category3Id = '')
    }
  },
  methods: {
    //向服务器发请求 获取Search数据 根据参数不同返回不同的数据
    getData() {
      this.$store.dispatch('getSearchList', this.searchParams)
    },
    // 删除关键字
    removeCategoryName() {
      ;(this.searchParams.categoryName = ''), (this.searchParams.category1Id = ''), (this.searchParams.category2Id = ''), (this.searchParams.category3Id = '')
      this.getData()
      //修改地址栏
